...
def _base_url(settings: BaseAppSettings) -> str:
    """
    Build base URL for links in emails.
    If you have settings like APP_BASE_URL / BASE_URL, adjust here.
    """
    return cast(str, getattr(settings, "APP_BASE_URL", "http://127.0.0.1:8000")).rstrip("/")
...
async def register_user(
    ...
    Sends activation email in background.
    ...
    # ✅ Email notification: activation request
    activation_link = (
        f"{_base_url(settings)}/api/v1/accounts/activate/?"
        f"email={new_user.email}&token={activation_token.token}"
    )
    background_tasks.add_task(
        email_sender.send_activation_request_email,  # ⚠️ звір назву у notifications/emails.py
        str(new_user.email),
        activation_link,
    )
...
async def activate_account(
    ...
    Sends activation complete email in background.
    """
    stmt = (
    ...
    # ✅ Email notification: activation complete
    login_link = f"{_base_url(settings)}/accounts/login/"
    background_tasks.add_task(
        email_sender.send_activation_complete_email,  # ⚠️ звір назву
        str(user.email),
        login_link,
    )
...
async def request_password_reset_token(
    ...
    Sends password reset request email in background (only if user exists and active).
    """
    ...
    reset_link = (
        f"{_base_url(settings)}/api/v1/accounts/reset-password/complete/?"
        f"email={user.email}&token={reset_token.token}"
    )
    background_tasks.add_task(
        email_sender.send_password_reset_request_email,  # ⚠️ звір назву
        str(user.email),
        reset_link,
    )
...
async def reset_password(
    ...
    Sends password reset complete email in background.
    """
    stmt = select(UserModel).filter_by(email=data.email)
    result = await db.execute(stmt)
    user = result.scalars().first()
    ...
    try:
        # ⚠️ якщо в тебе є метод set_password / PasswordManager — краще ним.
        # Тут залишаю як у твоєму коді, щоб мінімально міняти.
    ...
    # ✅ Email notification: password reset complete
    login_link = f"{_base_url(settings)}/accounts/login/"
    background_tasks.add_task(
        email_sender.send_password_reset_complete_email,  # ✅ згадано в умові
        str(user.email),
        login_link,
    )
